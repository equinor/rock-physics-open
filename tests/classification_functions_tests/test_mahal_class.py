...
def test_mahal_class(snapshot: SnapshotAssertion):
    ...
    assert snapshot == mahal_class(obs, class_mean, class_cov, class_id)
...
def test_mahal_class_thresh(snapshot: SnapshotAssertion):
    ...
    class_id = np.array([1, 2, 3])
    assert snapshot == mahal_class(obs, class_mean, class_cov, class_id, thresh=1.5)
